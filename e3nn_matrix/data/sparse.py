...
def csr_to_block_dict(spmat: sisl.SparseCSR, atoms: sisl.Atoms, nsc: np.ndarray, matrix_cls: Type[OrbitalMatrix] = OrbitalMatrix) -> OrbitalMatrix:
    """
    ...
    block_dict = _csr_to_block_dict(
        data=spmat.data[:, 0], ptr=spmat.ptr, cols=spmat.col,
        atom_first_orb=atoms.firsto, orbitals=orbitals,
        n_atoms=len(atoms.specie), 
    )

    return matrix_cls(block_dict=block_dict, nsc=nsc, orbital_count=orbitals)

def block_dict_to_coo(block_dict: Dict[Tuple[int, int, int], np.ndarray], first_orb: np.ndarray, n_supercells: int = 1, threshold: float = 1e-8) -> coo_array:
...
def block_dict_to_csr(block_dict: Dict[Tuple[int, int, int], np.ndarray], first_orb: np.ndarray, n_supercells: int = 1, threshold: float = 1e-8) -> csr_array:
    """Converts a block dictionary into a csr array.
    
    It just uses the conversion to coo, and then converts that to a csr array.
    """
    return block_dict_to_coo(block_dict=block_dict, first_orb=first_orb, n_supercells=n_supercells, threshold=threshold).tocsr()

def nodes_and_edges_to_coo(
    node_vals: ArrayLike, node_ptr: ArrayLike, 
    edge_vals: ArrayLike, edge_index: ArrayLike, edge_ptr: ArrayLike,
    orbitals: ArrayLike, 
    n_supercells: int = 1, edge_neigh_isc: Optional[ArrayLike] = None,
    threshold: float = 1e-8, symmetrize_edges: bool = False
) -> coo_array:
    ...
    for i_edge, ((i_at, j_at), neigh_isc) in enumerate(zip(edge_index.T, edge_neigh_isc)):
...
def nodes_and_edges_to_csr(
    ...
    It just uses the conversion to coo, and then converts that to a csr array.
    """
    return nodes_and_edges_to_coo(node_vals=node_vals, node_ptr=node_ptr, 
        edge_vals=edge_vals, edge_index=edge_index, edge_neigh_isc=edge_neigh_isc, edge_ptr=edge_ptr,
        orbitals=orbitals, n_supercells=n_supercells, threshold=threshold, symmetrize_edges=symmetrize_edges
    ).tocsr()

def csr_to_sisl_sparse_orbital(
    csr: csr_array, geometry: sisl.Geometry, sp_class: Type[SparseOrbital] = SparseOrbital
) -> SparseOrbital:
...
def nodes_and_edges_to_sparse_orbital(
    node_vals: ArrayLike, node_ptr: ArrayLike, 
    edge_vals: ArrayLike, edge_index: ArrayLike, edge_ptr: ArrayLike,
    geometry: sisl.Geometry, sp_class: Type[SparseOrbital] = SparseOrbital,
    edge_neigh_isc: Optional[ArrayLike] = None,
    threshold: float = 1e-8, symmetrize_edges: bool = False
) -> SparseOrbital:
    new_csr = nodes_and_edges_to_csr(
        node_vals=node_vals, node_ptr=node_ptr,
        edge_vals=edge_vals, edge_index=edge_index, edge_neigh_isc=edge_neigh_isc, edge_ptr=edge_ptr,
        orbitals=geometry.orbitals, n_supercells=geometry.n_s, threshold=threshold, symmetrize_edges=symmetrize_edges
    )
