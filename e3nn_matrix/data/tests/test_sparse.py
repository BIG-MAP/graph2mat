...
from e3nn_matrix.data.sparse import csr_to_block_dict, block_dict_to_csr, nodes_and_edges_to_csr, \
    csr_to_sisl_sparse_orbital, nodes_and_edges_to_sparse_orbital
...
def test_block_dict_to_csr_simple(density_matrix):
    ...
    new_csr = block_dict_to_csr(block_dict.block_dict, density_matrix.firsto, n_supercells=density_matrix.n_s)

    assert (new_csr.data != 0).sum() == 3
    for (i, j), val in zip([(0, 0), (0, first_orb_atom1), (first_orb_atom1, 0)], [1, 2, 3]):
...
def test_nodes_and_edges_to_csr(density_matrix, density_config, density_data, density_z_table):
    ...
    new_csr = nodes_and_edges_to_csr(
        density_data.atom_labels, atom_labels_ptr,
        density_data.edge_labels, density_data.edge_index, edge_labels_ptr,
        edge_neigh_isc=density_data.neigh_isc, n_supercells=density_matrix.n_s,
...
def test_nodes_and_edges_to_dm(density_matrix, density_config, density_data, density_z_table):
    ...
    new_dm = csr_to_sisl_sparse_orbital(new_csr, geometry=density_matrix.geometry, sp_class=sisl.DensityMatrix)
...
def test_nodes_and_edges_to_dm_direct(density_matrix, density_data, density_z_table):
    ...
    new_dm = nodes_and_edges_to_sparse_orbital(
        ...
        geometry=density_matrix.geometry, sp_class=sisl.DensityMatrix,
    )
