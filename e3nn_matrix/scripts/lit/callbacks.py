...
from e3nn_matrix.data.batch_utils import batch_to_sparse_orbital
...
class MatrixWriter(BasePredictionWriter):
    ...
    def write_on_batch_end(self,
        ...
        matrix_iter = batch_to_sparse_orbital(
            batch,
            z_table,
            matrix_cls,
            prediction,
        ...
        for data_idx, matrix in zip(batch_indices, matrix_iter):
            # Get the path from which this structure was read.
            path = paths[data_idx]

            # And write the matrix to it.
            matrix.write(path.parent / self.output_file)
