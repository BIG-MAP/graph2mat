...
def prepare_gen_dataset(
    dataset_dir: Annotated[str, typer.Option(help="Path to the directory where the dataset will be created/stored."
    " Note that this is relative to wherever you run SIESTA, so you might want to use an absolute path.")] = "MD_dataset",
    stepdir_prefix: Annotated[str, typer.Option(help="Prefix to add to the step directories. This is useful if you want to run multiple"
    " dataset generating runs and store them in the same directory.")] = "",
    store_interval: Annotated[int, typer.Option(help="Interval between two steps that are stored in the dataset.")] = 1,
    files_to_keep: Annotated[str, typer.Option(help="String containing the files that should be kept in the dataset. This is passed "
    "directly to the `cp` shell command, so you can, for example, use wildcards. If you use wildcards, make sure to wrap the "
    "input in single quotes (') to avoid the shell from expanding")] = "*fdf *TSHS *TSDE *XV",
    out: Annotated[str, typer.Option(help="Path where the resulting lua script should be stored.")] = "gen_dataset.lua"
):
