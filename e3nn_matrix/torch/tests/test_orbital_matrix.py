...
def test_orbital_matrix_data(density_matrix, density_data_processor):
    ...
    data = BasisMatrixTorchData.from_config(config, data_processor=density_data_processor)

    no_matrix_config = OrbitalConfiguration.from_geometry(geometry=density_matrix.geometry, matrix=block_dict)
    no_matrix_data = BasisMatrixTorchData.from_config(config, data_processor=density_data_processor)
